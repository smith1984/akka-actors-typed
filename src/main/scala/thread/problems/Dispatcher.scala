...
import thread.problems.Dispatcher.TaskDispatcher.LogWork
...
object Dispatcher extends App {
  ...
  def apply(): Behavior[NotUsed] =
    Behaviors.setup { ctx =>
      val dispatcherActorRef = ctx.spawn(TaskDispatcher(), "state_actor")

      dispatcherActorRef ! TaskDispatcher.LogWork("asdasdasd")
